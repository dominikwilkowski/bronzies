...
describe("The game", () => {
	...
	it("should count score and rounds in image2text mode", function () {
		...
		cy.get("[data-round]")
			...
			.get("[data-progress-status]")
			.should(($p) => {
				expect($p).to.have.length(SIGNALLENGTH);
				expect(Cypress.$($p[0]).attr("data-progress-status")).to.deep.eq(
					"current",
				);

				const rest = [...new Array(SIGNALLENGTH)].map((_, item) =>
					Cypress.$($p[item]).attr("data-progress-status"),
				);
				expect(rest).to.deep.eq([
					"current",
					...new Array(SIGNALLENGTH - 1).fill("future"),
				]);
			})
			...
			.each(() => {
				...
				cy.get('[data-question="true"] title')
					...
					.should(($p) => {
						expect($p).to.have.length(SIGNALLENGTH);
						expect(
							Cypress.$($p[position]).attr("data-progress-status"),
						).to.deep.eq("current");
						if (position > 0) {
							expect(
								Cypress.$($p[position - 1]).attr("data-progress-status"),
							).to.deep.eq("wrong");
						}
					})
					...
					.then(() => {
						score++;
						cy.contains("button[data-answer]", correct)
							.realClick()
							.then(() => {
								cy
									...
									.get("[data-answer]")
									.should("be.disabled")
									.get("[data-game-toggle]")
									.should("be.disabled")
									.get("[data-round-toggle]")
									.should("be.disabled")
			...
			.should(($p) => {
				position = 0;
				expect($p).to.have.length(SIGNALLENGTH);
				expect(Cypress.$($p[position]).attr("data-progress-status")).to.deep.eq(
					"current",
				);
				const rest = [...new Array(SIGNALLENGTH - position)].map((_, item) =>
					Cypress.$($p[item + position]).attr("data-progress-status"),
				);
				expect(rest).to.deep.eq([
					"current",
					...new Array(SIGNALLENGTH - position - 1).fill("future"),
				]);
			})
			...
			.each(() => {
				cy
					...
					.then(() => {
						...
						answerText = QUESTIONS.find(
							(question) => question.text === questionImageID,
						).image;
					})
					// we check the progress bar
					.get("[data-progress-status]")
					...
					.then(() => {
						cy.get(
							`[data-answer=""] [data-id]:not([data-id="${answerText}"])`,
							{ timeout: 60000 },
						)
							.eq(0)
							.realClick();
					...
					.then(() => {
						cy.get(`[data-answer=""] [data-id]:not([data-id="${answerText}"])`)
							.eq(1)
							.realClick();
					})
	...
	it("should keep track of your wrong answers", function () {
		...
		cy.get("[data-round]")
			...
			.get("[data-round-toggle]")
			.realClick()
			.get("[data-round-toggle-popup]")
			.then(($item) => {
				uniqeHistory = [...new Set(history)];
				expect($item, "text content").to.contain.text(
					`wrong so far (${uniqeHistory.length})`,
				);
			})
				...
				cy.wrap(uniqeHistory)
					.each(() => {
						cy
							...
							.then(() => {
								cy.get(
									`[data-answer=""] [data-id="${answerText}"]`,
								).realClick();
							})
							// next question
							.get('button[data-cy-id="Next question"]', { timeout: 60000 })
